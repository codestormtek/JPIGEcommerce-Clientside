...
- **Pages wired to API**: Homepage (featured products, bestsellers, blog, carousel), Shop listing (categories, brands, filtering, pagination), Product detail (server-side fetched, images, variants, attributes, add-to-cart), Header (dynamic categories, live search), Blog grid (`/blog`)
- **Product detail page**: `shop/[slug]/page.tsx` (server component) fetches product data server-side and passes to `ProductDetailClient.tsx` (client component). API response `{success, data}` is unwrapped in the server component. This avoids the infinite spinner that occurred with client-only useEffect fetching.
