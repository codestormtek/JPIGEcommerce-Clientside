...
export function Working() {
  ...
  const groupedProducts = categories.map(category => ({
    ...category,
    products: config.menu.products.filter(product => product.primaryCategoryId === category.id || product.categoryIds.includes(category.id)),
  })).filter(category => category.products.length);
  const uncategorized = config.menu.products.filter(product => !groupedProducts.some(category => category.products.some(item => item.id === product.id)));
  const visibleGroups = activeCategory === "all"
    ? groupedProducts
    : groupedProducts.filter(category => category.id === activeCategory);
  const productCard = (product: KioskProduct) => {
    const price = product.items[0]?.price ?? 0;
    return <button key={product.id} className="jp-product" onClick={() => openProduct(product)}>
      <div className="jp-product-img"><ProductArt name={product.name} />{product.comboSideCount ? <span>{product.comboSideCount} sides</span> : null}</div>
      <div><h2>{product.name}</h2><p>{product.description || (product.comboSideCount ? "Pick your favorite sides" : "Straight from the Jiggling Pig pit")}</p><footer><b>{formatMoney(price)}</b><i aria-hidden="true">+</i></footer></div>
    </button>;
  };
  const menu = <section className="jp-menu">
    <div className="jp-menu-heading"><p className="jp-kicker">Roadside pickup</p><h1>What’s<br /><em>smoking?</em></h1><p>{config.eventName} <span>·</span> about {config.asapWaitMinutes} min</p></div>
    ...
    <div className="jp-category-groups">
      {visibleGroups.map(category => <section className="jp-category-section" id={`category-${category.id}`} key={category.id}><div className="jp-category-title"><h2>{category.name}</h2><span>{category.products.length} item{category.products.length === 1 ? "" : "s"}</span></div><div className="jp-grid">{category.products.map(productCard)}</div></section>)}
      {activeCategory === "all" && uncategorized.length > 0 && <section className="jp-category-section"><div className="jp-category-title"><h2>More from the pit</h2></div><div className="jp-grid">{uncategorized.map(productCard)}</div></section>}
      {!visibleGroups.length && <div className="jp-empty jp-menu-empty"><span>00</span><p>Nothing is listed in this section yet.</p></div>}
    </div>
  </section>;
