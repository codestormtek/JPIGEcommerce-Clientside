      ...
      <Content>
        ...
        <Block>
          ...
          <div className="card card-bordered mb-4"><div className="card-inner">
            ...
            <form onSubmit={saveConfig}><input className="form-control mb-3" required type="url" value={config.canonicalOrigin || ""} onChange={(event) => setConfig((current) => ({ ...current, canonicalOrigin: event.target.value }))} placeholder="https://your-stable-domain.example" aria-label="Canonical Smart Links origin" />
              <textarea className="form-control mb-2" rows="3" value={allowedOriginsText} onChange={(event) => setAllowedOriginsText(event.target.value)} placeholder={"Optional exact HTTPS destination origins, one per line\nhttps://another-approved-domain.example"} aria-label="Allowed external destination origins" />
          ...
          <div className="card card-bordered mb-4"><div className="card-inner">
            <h5>New smart link</h5><form className="mt-3" onSubmit={create}><Row className="g-3">
              <Col md="6"><input className="form-control" required maxLength="120" value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} placeholder="Campaign name" aria-label="Campaign name" /></Col>
              <Col md="6"><input className="form-control" required maxLength="80" pattern="[a-z0-9]+(-[a-z0-9]+)*" value={form.slug} onChange={(event) => setForm((current) => ({ ...current, slug: event.target.value.toLowerCase().replace(/\s+/g, "-") }))} placeholder="spring-festival" aria-label="Permanent slug" /></Col>
              <Col md="6"><input className="form-control" required type="url" value={form.targetUrl} onChange={(event) => setForm((current) => ({ ...current, targetUrl: event.target.value }))} placeholder="https://your-approved-domain.example/order" aria-label="Active destination URL" /></Col>
              <Col md="6"><input className="form-control" type="url" value={form.fallbackUrl} onChange={(event) => setForm((current) => ({ ...current, fallbackUrl: event.target.value }))} placeholder="Optional approved fallback after archive" aria-label="Archive fallback URL" /></Col>
...
const LinkCard = ({ link, busy, editing, editForm, setEditForm, onStartEdit, onSaveEdit, action, copy, download, preview, metrics, history, loadMetrics, loadHistory, reload }) => {
  ...
  return <div className={`card card-bordered mb-4 ${link.isArchived ? "opacity-75" : ""}`}><div className="card-inner">
    ...
    {editing && !link.isArchived && <form className="border rounded bg-light p-3 mt-4" onSubmit={(event) => onSaveEdit(event, link)}><Row className="g-3"><Col md="6"><input className="form-control" required maxLength="120" value={editForm.title} onChange={(event) => setEditForm((current) => ({ ...current, title: event.target.value }))} /></Col><Col md="6"><input className="form-control" disabled value={link.slug} title="Slugs are permanent and cannot be changed" /></Col><Col md="6"><input className="form-control" type="url" required value={editForm.targetUrl} onChange={(event) => setEditForm((current) => ({ ...current, targetUrl: event.target.value }))} /></Col><Col md="6"><input className="form-control" type="url" value={editForm.fallbackUrl} placeholder="Optional archive fallback" onChange={(event) => setEditForm((current) => ({ ...current, fallbackUrl: event.target.value }))} /></Col><Col><p className="form-note text-soft">Slug is immutable. Set the link active before editing.</p><Button size="sm" color="primary" type="submit" disabled={disabled}>Save changes</Button><Button size="sm" color="light" outline className="ms-2" type="button" onClick={() => onStartEdit(link)}>Cancel</Button></Col></Row></form>}
    <div className="d-flex gap-2 flex-wrap border-top pt-3 mt-4">
      ...
      {!link.isArchived && <Button size="sm" color="light" outline disabled={disabled} onClick={() => onStartEdit(link)}>{editing ? "Close edit" : "Edit"}</Button>}
